...
func TestAssignFlavors(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			updateResources(clusterQueue.GuaranteedQuota, tc.clusterQueueGuaranteedQuota)
...
func updateResources(dest resources.FlavorResourceQuantities, src resources.FlavorResourceQuantitiesFlat) {
	...
	for flavorResource, usage := range src {
		if _, ok := dest[flavorResource.Flavor]; !ok {
			dest[flavorResource.Flavor] = make(workload.Requests)
		}
		dest[flavorResource.Flavor][flavorResource.Resource] += usage
	}
}
